...
<!--  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="矿池AccessKey(加密)" prop="poolAccessKey">
        <el-input
          v-model="queryParams.poolAccessKey"
          placeholder="请输入矿池AccessKey(加密)"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item label="矿池私钥(加密)" prop="poolPrivateKey">
        <el-input
          v-model="queryParams.poolPrivateKey"
          placeholder="请输入矿池私钥(加密)"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['project:mining-config:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['project:mining-config:export']"
        >
...
<!--      <el-table-column label="币种" align="center" prop="coin" />
      <el-table-column label="矿池AccessKey(加密)" align="center" prop="poolAccessKey" />
      <el-table-column label="矿池私钥(加密)" align="center" prop="poolPrivateKey" />
      <el-table-column label="矿池子账号" align="center" prop="poolName" />
      <el-table-column label="总算力(T)" align="center" prop="computingPower" />-->
      <el-table-column label="单T功耗(W)" align="center" prop="powerConsumption" />
      <el-table-column label="电价(元/度)" align="center" prop="electricityPrice" />
...
<!--      <el-table-column label="运维成本类型:0-固定值/月 1-按天" align="center" prop="operationCostType" />
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['project:mining-config:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['project:mining-config:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <MiningConfigForm ref="formRef" @success="getList" />
</template>
...
import MiningConfigForm from './MiningConfigForm.vue'

/** 挖矿项目配置 列表 */
defineOptions({ name: 'ProjectMiningConfig' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<MiningConfigVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await MiningConfigApi.getMiningConfigPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await MiningConfigApi.deleteMiningConfig(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await MiningConfigApi.exportMiningConfig(queryParams)
    download.excel(data, '挖矿项目配置.xls')
  } catch {
  } finally {
