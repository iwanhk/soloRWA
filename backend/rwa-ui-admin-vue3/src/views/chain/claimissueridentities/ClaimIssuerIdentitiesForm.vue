  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="发行者地址" prop="address">
        <el-input v-model="formData.address" placeholder="请输入发行者地址" />
      </el-form-item>
      <el-form-item label="发行者合约地址" prop="contractAddress">
        <el-input v-model="formData.contractAddress" placeholder="请输入发行者合约地址" />
      </el-form-item>
...
/** 声明发行者身份 表单 */
defineOptions({ name: 'ClaimIssuerIdentitiesForm' })
...
const formRules = reactive({
  address: [{ required: true, message: '发行者地址不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ClaimIssuerIdentitiesApi.getClaimIssuerIdentities(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as ClaimIssuerIdentitiesVO
    if (formType.value === 'create') {
      await ClaimIssuerIdentitiesApi.createClaimIssuerIdentities(data)
      message.success(t('common.createSuccess'))
    } else {
      await ClaimIssuerIdentitiesApi.updateClaimIssuerIdentities(data)
