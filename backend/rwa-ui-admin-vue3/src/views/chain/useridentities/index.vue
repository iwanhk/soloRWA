<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="关联的代币ID，JSON格式" prop="associatedTokenIds">
        <el-input
          v-model="queryParams.associatedTokenIds"
          placeholder="请输入关联的代币ID，JSON格式"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item label="待处理的代币ID，JSON格式" prop="pendingTokenIds">
        <el-input
          v-model="queryParams.pendingTokenIds"
          placeholder="请输入待处理的代币ID，JSON格式"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['chain:user-identities:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['chain:user-identities:export']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="用户合约地址" align="center" prop="contractAddress" />
      ...
      <el-table-column label="关联的代币ID，JSON格式" align="center" prop="associatedTokenIds" />
      <el-table-column label="待处理的代币ID，JSON格式" align="center" prop="pendingTokenIds" />
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['chain:user-identities:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['chain:user-identities:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <UserIdentitiesForm ref="formRef" @success="getList" />
</template>
...
import UserIdentitiesForm from './UserIdentitiesForm.vue'
...
const list = ref<UserIdentitiesVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await UserIdentitiesApi.getUserIdentitiesPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await UserIdentitiesApi.deleteUserIdentities(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await UserIdentitiesApi.exportUserIdentities(queryParams)
    download.excel(data, '用户身份.xls')
  } catch {
  } finally {
