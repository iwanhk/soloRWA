<template>
  <div class="upload-box">
    ...
    >
      <template v-if="modelValue && !isUploading">
      ...
      <template v-else-if="isUploading">
        <div class="upload-loading">
          <el-progress 
            ...
            :width="progressWidth"
          />
          <span class="loading-text">{{ percentage }}%</span>
        </div>
      </template>
      <template v-else>
...
const props = defineProps({
  ...
  fileSize: propTypes.number.def(10), // 图片大小限制 ==> 非必传（默认为 5M）
...
const handleProgress = (evt) => {
  percentage.value = Math.round(evt.percent || 0)
}
