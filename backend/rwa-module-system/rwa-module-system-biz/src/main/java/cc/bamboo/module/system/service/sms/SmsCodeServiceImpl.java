...
import cc.bamboo.module.system.api.sms.dto.code.SmsCodeValidateReqDTO;
import cc.bamboo.module.system.dal.dataobject.sms.SmsCodeDO;
import cc.bamboo.module.system.dal.mysql.sms.SmsCodeMapper;
...
import static cc.bamboo.framework.common.util.date.DateUtils.isToday;
...
public class SmsCodeServiceImpl implements SmsCodeService {
    ...
    public void sendSmsCode(SmsCodeSendReqDTO reqDTO) {
        ...
        if (Boolean.TRUE.equals(smsCodeProperties.getEnableSend())) {
            ...
            log.info("[sendSmsCode][跳过短信发送] mobile: {}, code: {}, scene: {}",
                    reqDTO.getMobile(), code, reqDTO.getScene());
        }
    }
    ...
    private String createSmsCode(String mobile, Integer scene, String ip) {
        ...
        if (lastSmsCode != null) {
            if (LocalDateTimeUtil.between(lastSmsCode.getCreateTime(), LocalDateTime.now())
                    .toMillis() < smsCodeProperties.getSendFrequency().toMillis()) { // 发送过于频繁
        ...
        SmsCodeDO newSmsCode = SmsCodeDO.builder().mobile(mobile).code(code).scene(scene)
                .todayIndex(
                        lastSmsCode != null && isToday(lastSmsCode.getCreateTime()) ? lastSmsCode.getTodayIndex() + 1
                                : 1)
    ...
    private SmsCodeDO validateSmsCode0(String mobile, String code, Integer scene) {
        ...
        // 超过时间
        if (LocalDateTimeUtil.between(lastSmsCode.getCreateTime(), LocalDateTime.now()).toMillis() >= smsCodeProperties
                .getExpireTimes().toMillis()) { // 验证码已过期
