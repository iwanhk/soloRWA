...
import cc.bamboo.module.chain.service.identity.dto.DeleteWalletRespDTO;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        name = "user.remove.queue",
                        durable = "true"
                ),
                exchange = @Exchange(
                        name = "user.remove.exchange",
...
public class UserRemoveWalletConsumer {
     ...
     */
    @RabbitHandler
    public void onMessage(RemoveAddressMessage message) {
        String taskNo = message.getTaskNo();
        Integer retryCount = message.getRetryCount() != null ? message.getRetryCount() : 0;

        log.info("[RemoveAddressConsumer] 收到用户解绑钱包地址消息，taskNo: {}, 用户地址: {}, 重试次数: {}",
                taskNo,  message.getUserAddress(), retryCount);
        ...
        if (task == null) {
            log.error("[RemoveAddressConsumer] 任务创建失败，taskNo: {}", taskNo);
            return;
        }

        // 2. 检查任务状态（幂等性）
        if (ChainTaskStatusEnum.SUCCESS.getStatus().equals(task.getStatus())) {
            log.info("[RemoveAddressConsumer] 任务已成功，跳过执行，taskNo: {}", taskNo);
            return;
        }
        ...
        try {
            ...
            DeleteWalletRespDTO deleteWallet = identityService.deleteWallet(
            ...
            // 6. 更新任务状态为成功
            chainOperationTaskService.updateTaskToSuccess(taskNo, JSONUtil.toJsonStr(executionSteps),deleteWallet.getTransactionHash());

            // 7.更新该地址为绑定成功状态
            tokensMapper.removeUserChainAddress(ChainAddressStatusEnum.UNBIND.getStatus(),  message.getUserChainId());

            log.info("[RemoveAddressConsumer] 用户添加地址，taskNo: {},  用户地址: {}",
                    taskNo,  message.getUserAddress());

        } catch (Exception e) {
            executionSteps.add("执行失败: " + e.getMessage());
            log.error("[RemoveAddressConsumer] 用户添加地址，taskNo: {},  错误: {}",
            ...
            if (nextRetryCount <= MAX_RETRY_COUNT) {
                ...
                log.info("[RemoveAddressConsumer] 已发送重试消息，taskNo: {}, 重试次数: {}/{}", 
                ...
                tokensMapper.updateUserChainAddress(ChainAddressStatusEnum.UNBIND_FAIL.getStatus(),  message.getUserChainId());
                log.error("[RemoveAddressConsumer] 任务最终失败，taskNo: {}, 已重试 {} 次", taskNo, MAX_RETRY_COUNT);
            }
        }
    }
