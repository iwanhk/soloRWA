...
import cc.bamboo.module.chain.mq.producer.ClaimProducer;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        name = "user.create.queue",
                        durable = "true"
                ),
                exchange = @Exchange(
                        name = "user.create.exchange",
...
public class UserCreateConsumer {
    ...
    public void onMessage(UserCreateMessage message) {
        ...
        log.info("[UserCreateConsumer] 收到用户绑定钱包地址消息，taskNo: {}, 用户ID: {}, 重试次数: {}",
        ...
        if (task == null) {
            log.error("[UserCreateConsumer] 任务创建失败，taskNo: {}", taskNo);
            return;
        }

        // 2. 检查任务状态（幂等性）
        if (ChainTaskStatusEnum.SUCCESS.getStatus().equals(task.getStatus())) {
            log.info("[UserCreateConsumer] 任务已成功，跳过执行，taskNo: {}", taskNo);
            return;
        }
        ...
        try {
            ...
            // 7.更新该地址为绑定成功状态
            tokensMapper.createUserChainAddress(ChainAddressStatusEnum.SUCCESS.getStatus(),  message.getUserChainId(),addUserResp.getId());

             // 8. 检查用户是否需要审核
             int auditStatus = tokensMapper.getUserAuditStatus(message.getUserId());
             if (AuditStatusEnum.APPROVED.getStatus().equals(auditStatus)) {
                 // 9. 发送 Claim 签发消息
                 claimProducer.sendUserClaimMessage(message.getUserId());
             }

            log.info("[UserCreateConsumer] 用户添加成功，taskNo: {},  用户地址: {}",
                    taskNo,  message.getUserAddress());

        } catch (Exception e) {
            executionSteps.add("执行失败: " + e.getMessage());
            log.error("[UserCreateConsumer] 用户添加和 Claim 签发失败，taskNo: {},  错误: {}",
            ...
            if (nextRetryCount <= MAX_RETRY_COUNT) {
                ...
                log.info("[UserCreateConsumer] 已发送重试消息，taskNo: {}, 重试次数: {}/{}", 
                ...
                tokensMapper.createUserChainAddress(ChainAddressStatusEnum.BIND_FAIL.getStatus(),  message.getUserChainId(),null);
                log.error("[UserCreateConsumer] 任务最终失败，taskNo: {}, 已重试 {} 次", taskNo, MAX_RETRY_COUNT);
            }
        }
    }
