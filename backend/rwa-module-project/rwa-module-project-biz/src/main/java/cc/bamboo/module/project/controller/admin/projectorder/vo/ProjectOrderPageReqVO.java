...
@Schema(description = "管理后台 - 项目认购订单分页 Request VO")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public class ProjectOrderPageReqVO extends PageParam {

    @Schema(description = "订单号（唯一，如：ORD202512160001）")
    ...
    @Schema(description = "用户ID（关联用户表）", example = "15701")
    private Long userId;

    @Schema(description = "订单状态：0-待支付 1-审核中 2-审核通过 3-审核未通过 4-已取消", example = "1")
    private Integer orderStatus;

    @Schema(description = "项目ID（关联project_core.id）", example = "14723")
    private Long projectId;

    @Schema(description = "项目名称（冗余）", example = "芋艿")
    private String projectName;

    @Schema(description = "申购份额(份)")
    private Integer subscribeQuantity;

    @Schema(description = "单价", example = "7252")
    private BigDecimal price;

    @Schema(description = "金额(元)（=申购份额×项目发行单价）")
