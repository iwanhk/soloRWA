...
class ChartHelper {
  ...
  drawAxis(ctx, grid, canvasWidth, canvasHeight, xAxisData, minY, maxY, xAxisOpt, yAxisOpt, chartType = 'line', xAxisPadding = 0) {
    ...
    if (showYAxisLabel !== undefined ? showYAxisLabel : false) {
        ...
        ctx.textAlign = yAxisOpt.axisLabel?.align??'right';
        ctx.textBaseline = 'middle';
        
        const xMargin = yAxisOpt.axisLabel?.marginLeft ?? 0;
        const yMargin = yAxisOpt.axisLabel?.marginTop ?? 0;
        const x = (yAxisOpt.axisLabel?.inside? 0 : (grid.left || 0) - 10)+xMargin;
        const formatter = yAxisOpt.axisLabel?.formatter;
        
        // 绘制Y轴标签 (使用调整后的Y轴范围)
        for (let i = 0; i <= yAxisTicks.tickCount; i++) {
            const value = yAxisTicks.min + i * yAxisTicks.step;
            const ratio = (value - yAxisTicks.min) / (yAxisTicks.max - yAxisTicks.min);
            const y = ((grid.top || 0) + chartHeight - ratio * chartHeight)+yMargin;
            ctx.fillText(formatter?formatter(value):Math.round(value).toString(), x, y);
        }
    }
  }
