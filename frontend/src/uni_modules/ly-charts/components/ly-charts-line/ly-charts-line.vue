...
const drawChart = (option) => {
  ...
  try {
	...
	if(option.grid){
		grid.value = Object.assign(grid.value, option.grid);
	}
...
const drawSeries = (series, xAxisData, minY, maxY, adjustedYMin, adjustedYMax, xAxisPadding = 0) => {
  ...
  series.forEach((serie, index) => {
    ...
    const color = serie?.areaStyle || serie.color || serie.itemStyle?.color || chartHelper.getColor(index);
  ...
  series.forEach((serie, index) => {
    ...
    if (serie.data && Array.isArray(serie.data)) {
      serie.data.forEach((value, i) => {
        const actualValue = typeof value === 'object' && value !== null ? value.value : value;
		const displayValue = serie.label.formatter?serie.label.formatter(actualValue):actualValue.toString();
        // 确保不会除以零，并应用X轴padding
        const x = grid.value.left + xAxisPadding + (xAxisData.length > 1 ? (i / (xAxisData.length - 1)) * paddedChartWidth : 0);
        // 使用调整后的Y轴范围计算Y坐标
        const y = grid.value.top + chartHeight - ((actualValue - actualMinY) / (actualMaxY - actualMinY || 1)) * chartHeight;
...
const drawFillArea = (points, color, smooth, chartBottom) => {
  ...
  function parseColor(color){
	  if (typeof color === 'string' && color.startsWith('#')) {
		  ...
		  const a = parseInt(hex.substring(6, 8), 16) || 0.3;
		  return `rgba(${r}, ${g}, ${b}, ${a})`
  ...
  color = color.map(c=>parseColor(c));
  if(color.length === 1){
	  ctx.value.setFillStyle(color[0])
  }
  else{
	  const g = ctx.value.createLinearGradient(0, 0, 0, chartBottom );

	  color.forEach((c, index) => {
		  g.addColorStop(index/(color.length - 1), c);
	  })

	  ctx.value.setFillStyle(g);
  }
...
const drawLabels = (points, color, position) => {
  ...
  points.forEach(point => {
    const label = point.displayValue;
